...
public class ScoreServiceV3 {
	...
	public int insert(ScoreDTO scoreDTO) { // 다오한테 중계해주는 역할만 수행,, service가 하는 일
	...
	public String inputStudent() {
		...
		if(strStNum.equals("-Q")) return null; // Q가 나오면 종료,, 아니면 다음으로 진행
		return strStNum; // -Q를 입력했다면 null을 return, 그렇지 않다면 strStNum을 return
	}
	 ...
	 * Integer 클래스 형은 0~9까지 숫자형과 null을 사용가능
	 */
	...
	public int updateScore(ScoreVO scoreVO) {
		...
		// 그렇지 않으면 무시
		if(strStNum.length() > 0) { //입력을 했었다는 표시,, 값을 입력했을 때는 VO에 있는 학번을 바꾸고
		...
		try {
			...
			scoreVO.setS_score(intScore); // Exception이 나오면 VO를 세팅하는 이 부분을 건너 뜀
		} catch (Exception e) {
