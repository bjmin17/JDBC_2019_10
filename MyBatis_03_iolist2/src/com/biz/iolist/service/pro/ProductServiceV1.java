...
//		1번째 방법
//		String strVAT = proDTO.getP_vat() != null
...
//		if(strName.trim().length()<1) 
		 ...
		 * 
		 * 이런 기능을 역 이용하여
		 * 값을 입력하지 않거나, 문자열 등을 포함하면
		 * 원래 proDTO의 iprice 값을 변경하지 않도록 한다.
		 */
		// strIPrice를 숫자로 바꾸려고 하는데 아무 것도 없으니 catch로 넘어가고
		// catch도 없으니 밖으로 나가서 그대로 쭈욱 실행,,, 결국, IPrice는 바뀌지 않음
		try {
		...
		/*
		 *  proDTO의 필드변수들은
		 *  각 항목을 입력받을 때 Enter만 입력했다면
		 *  원래 DB table에서 가져온 값이 그대로 유지될 것이고
		 *  
		 *  값을 입력했다면
