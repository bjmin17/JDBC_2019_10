...
public class BookServiceV1 {
	...
	private void viewList(List<BookDTO> bookList) {
		...
		System.out.println("-------------------------------");
		for(BookDTO dto : bookList) { // bookList로부터 전체리스트를 가져와서 BookDTO형식의 dto에 담겠다
	...
	public void searchBookName(boolean bConti) {
		...
		while(true) {
			if(this.searchBookName() != null) break; // true(-Q를 입력하거나 정상적으로 검색되면)이면 끝내라,,
...
//				continue;
		...
		// bookList에는 입력한 도서명에 해당하는 리스트가 담겨 있을 것이다.
